describe('login', () => {
  ...
  it('Should not login if only  password is inputted', () => {
    cy.visit('/')
    cy.get('[data-cy="Log In"]').click()
  ...
  it('Should not let user Sign up with not all fields inputted ', () => {
    cy.visit('/')
    cy.get('[data-cy="Log In"]').click()
    cy.url().should('includes', 'login');
    cy.get('[data-cy="Sign-Up"]').click()
  ...
  it('Should not let user Sign up when passwords don\'t match ', () => {
    cy.visit('/')
    cy.get('[data-cy="Log In"]').click()
    cy.url().should('includes', 'login');
    cy.get('[data-cy="Sign-Up"]').click()
    ...
    cy.get('[formControlName="email"]').type('bob@gmail.com');
    cy.get('[formControlName="password"]').type('bobbyIsTheBest');
    cy.get('[formControlName="reenteredPass"]').type('bobbyIsTheBestest!!!');
  ...
  it('Should let user sign up... clicking \"Sign Up\" from login page', () => {
    ...
    cy.get('[data-cy="testFirstName"]').type('Bobby');
    cy.get('[data-cy="testLastName"]').type('Fergison');
    cy.get('[formControlName="email"]').type('bobbbb@gmail.com');
    cy.get('[formControlName="password"]').type('bobbyIsTheBest');
    cy.get('[formControlName="reenteredPass"]').type('bobbyIsTheBest');
    cy.url().should('includes', 'sign-up');
    cy.get('[data-cy="Submit Sign-Up"]', { timeout: 20000 }).click();
    cy.url().should('include', 'login');

  })

  //closes out tab and logs in 
  //test logging out
